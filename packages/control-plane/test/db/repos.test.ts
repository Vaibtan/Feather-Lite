  ...
  it("claims due scheduled actions and outbox jobs with SKIP LOCKED, once each", async () => {
    const out = await rt.runPromise(
      Effect.gen(function* () {
        ...
        yield* conv.insertConversation({ id: convId, callAttemptId: attemptId, borrowerId, agentVersionId: version.id, startedAt: new Date(), channel: "simulated", origin: "simulated", harness: null, decider: "scripted" });
